...
**Branch**: `123-physical-ai-textbook` | **Date**: 2026-03-04
**Input**: Feature specification from `/specs/textbook/spec.md`

## Summary

This plan outlines the implementation of a Physical AI & Humanoid Robotics textbook using Docusaurus, deployed on GitHub Pages, with an embedded RAG chatbot powered by free-tier services (Google Gemini, Qdrant Cloud, Neon Postgres, Render.com). The project includes core textbook content, a chatbot for Q&A, and bonus features like authentication, content personalization, and Urdu translation.

## Technical Context

**Language/Version**: Python 3.11+ (backend), Node.js 20+ (frontend), TypeScript (Docusaurus components)
**Primary Dependencies**: Docusaurus 3, FastAPI, LangChain, Qdrant client, Neon Postgres client, better-auth, Google Gemini API client, pnpm
**Storage**: Qdrant Cloud (vector database), Neon Serverless Postgres (user data)
**Testing**: pytest (backend), potentially Jest/Vitest (frontend), contract tests
**Target Platform**: Web browser (Docusaurus), Linux server (Render.com backend)
**Project Type**: Web application
**Performance Goals**: RAG response < 5 seconds (p95), Gemini API rate limits handled, Render.com free tier spin-down state managed.
**Constraints**: Strictly use free-tier services, public GitHub repository, handle Gemini API limits (15 req/min, 1M tokens/day).
**Scale/Scope**: Full textbook content, estimated ~1M tokens for embeddings, moderate user load for chatbot.

## Constitution Check

*GATE: Must pass before Phase 0 research. Re-check after Phase 1 design.*

- **V. Simplicity**: The chosen stack prioritizes open-source and free tiers, aligning with simplicity. Complexity is managed by using established libraries like LangChain.
- **VI. Versioning & Breaking Changes**: Will be managed via Git branching and Docusaurus versioning features.
- **VII. Observability**: Logging will be implemented in FastAPI backend and tracked via Render.com.
...
backend/                       # FastAPI RAG backend
    ...
    └── requirements.txt
scripts/
    └── ingest_book.py             # Script to embed docs into Qdrant
.github/
    └── workflows/
...
**Structure Decision**: Using a monorepo structure with `book/` for Docusaurus frontend and `backend/` for FastAPI backend, aligning with the provided project structure.
